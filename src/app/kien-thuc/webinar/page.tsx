...
import { WEBINARS, formatDate } from "../libs/mock-data";
...
export default function WebinarPage() {
  ...
  const categories = useMemo(
    () => Array.from(new Set(WEBINARS.map((w) => w.category))),
    [],
  );
  ...
  const stats = [
    { value: WEBINARS.length, suffix: "", label: "Webinar" },
    { value: upcomingCount, suffix: "", label: "Sắp diễn ra" },
    { value: recordedCount, suffix: "", label: "Đã ghi hình" },
  ...
  return (
    <div className="bg-white">
          ...
          <div className="max-w-[1400px] w-full mx-auto px-6 lg:px-12">
            ...
            >
              ...
              <p className="mt-5 md:mt-6 text-base md:text-lg text-white/80 leading-relaxed max-w-[60ch]">
                Hội thảo trực tuyến cùng chuyên gia Kepler về thị trường, đầu tư,
                pháp lý, quy hoạch và chuyển đổi — trực tiếp và theo yêu cầu.
              </p>
      ...
      {/* ===== 3. WEBINAR LIST ===== */}
      <section id="webinars" className="bg-gray-50 py-16 pt-20 scroll-mt-20">
        <div className="max-w-[1200px] mx-auto px-6 lg:px-12">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              key={view}
              className={view === "list" ? "space-y-4" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"}
              initial="hidden"
              animate="visible"
              exit="hidden"
              variants={{
            ...
            >
              {visible.map((webinar) => (
                ...
                >
                  {view === "list" ? (
                    <Link
                      href={`/kien-thuc/webinar/${webinar.slug}`}
                    ...
                    >
                      {/* Thumbnail */}
                      <div className="relative w-40 h-24 md:w-52 md:h-32 rounded-lg overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-red-900 shrink-0">
                        <div
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="w-10 h-10 rounded-full bg-white/15 backdrop-blur-sm flex items-center justify-center group-hover:bg-white group-hover:scale-110 transition-all">
                            {webinar.status === "upcoming" ? (
                        ...
                        <div className="absolute bottom-1.5 right-1.5 z-10">
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-black/50 backdrop-blur-sm text-white text-[10px] font-medium rounded">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-500 text-sm line-clamp-1 mb-2">{webinar.description}</p>
                    ...
                    >
                      {/* Thumbnail */}
                      <div className="relative h-44 bg-gradient-to-br from-gray-900 via-gray-800 to-red-900 overflow-hidden">
                        <div
                          className="absolute inset-0 opacity-20"
                          style={{
                            backgroundImage:
                        ...
                        <div className="absolute top-3 right-3 z-10">
                          <span className="inline-flex items-center px-2.5 py-1 bg-white/10 backdrop-blur-sm text-white text-xs font-medium rounded-full border border-white/10">
                            {webinar.category}
                          </span>
                        </div>
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="w-14 h-14 rounded-full bg-white/15 backdrop-blur-sm flex items-center justify-center group-hover:bg-white group-hover:scale-110 transition-all">
                            {webinar.status === "upcoming" ? (
                              <Calendar className="w-6 h-6 text-white group-hover:text-red-600 transition-colors" />
                            ) : (
                        ...
                        <div className="absolute bottom-3 right-3 z-10">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-black/50 backdrop-blur-sm text-white text-xs font-medium rounded">
                            <Clock className="w-3 h-3" />
                            {webinar.duration}
                          </span>
                        </div>
          ...
          {filtered.length === 0 && (
            <div className="py-16 text-center bg-white rounded-lg border border-gray-100 shadow-sm">
              <p className="text-gray-500 text-sm">
                Chưa có webinar trong chủ đề này.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          >
            ...
            <p className="mt-6 text-gray-400 text-base leading-relaxed max-w-[60ch]">
              Kepler cung cấp nhiều định dạng nội dung để bạn tiếp cận kiến thức
              theo cách phù hợp nhất — video ngắn, podcast và webinar.
            </p>
