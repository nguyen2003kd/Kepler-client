...
export async function generateMetadata({ params }: NewsSubPageProps): Promise<Metadata> {
...
export default async function NewsSubPage({ params, searchParams }: NewsSubPageProps) {
  const fullSlug = `${PREFIX}/${params.slug.join("/")}`;
  const lastSlug = params.slug.at(-1)!;
  ...
  if (post) {
    ...
    return (
      <DynamicPostDetailPage
        post={post}
        categoryName={parentCategory?.name || "Tin tức"}
